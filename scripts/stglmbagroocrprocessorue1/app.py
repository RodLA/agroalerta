...
# --- Variables IA ---
# CAMBIO APLICADO: Ya no leemos la llave directamente, sino el nombre del secreto
SECRET_NAME_GEMINI = os.environ.get('SECRET_NAME_GEMINI')
MODEL_ID = os.environ.get('MODEL_ID', 'gemini-2.0-flash') 
GEMINI_API_KEY = None # Se llenará en la etapa de inicialización (Cold Start)
...
MONGO_COLLECTION_NAME = os.environ.get('MONGO_COLLECTION_NAME', 'documents_risk_alerts')
MONGO_COLLECTION_RECOMMENDATIONS = os.environ.get('MONGO_COLLECTION_RECOMMENDATIONS', 'alerts_recommendations')
...
# --- NUEVO: Colección de Dominios Senamhi ---
...
# --- CAMBIO APLICADO: Configurar Gemini usando Secrets Manager ---
try:
    if SECRET_NAME_GEMINI:
        gemini_secret = get_secret(SECRET_NAME_GEMINI)
        GEMINI_API_KEY = gemini_secret.get('api_key')
        
        if GEMINI_API_KEY and GEMINI_API_KEY != "dummy_key":
            genai.configure(api_key=GEMINI_API_KEY)
            logger.info("🤖 [INIT] API Key de Gemini configurada exitosamente.")
        else:
            logger.warning("⚠️ [INIT] La API Key de Gemini aún tiene el valor dummy o está vacía.")
    else:
        logger.warning("⚠️ [INIT] No se proporcionó SECRET_NAME_GEMINI en las variables de entorno.")
except Exception as e:
    logger.error(f"❌ [INIT] Fallo al cargar el secreto de Gemini: {e}")
...
def get_pdf_metadata(s3_key: str) -> dict:
    if not s3_key: 
        return {}
    try:
        # Limpia espacios invisibles y saltos de línea
        clean_key = s3_key.strip()
        filename = clean_key.split('/')[-1]
        col = db_global[TABLE_METADATA_NAME]

        doc = col.find_one({"url": {"$regex": f"{re.escape(filename)}$"}})
    ...
    {{
        ...
        "enriched_data": [
            {{
                ...
                "description": "<Un resumen analítico y detallado del contexto del boletín respecto a los departamentos, provincias y dominios listados. Indica claramente la situación general y climática.>",
                "summary": "<Un párrafo neutral (máx 3 oraciones) dirigiéndose al agricultor. Menciona su cultivo, fecha, nivel de riesgo y qué significa en la práctica. Cero tecnicismos.>"
            }}
        ],
        "recommendations": [
            {{
...
def handler(event, context):
    ...
    try:
        ...
        logger.info("🔍 [ETAPA 2] Consultando metadatos en MongoDB por nombre de archivo...")
        metadata = get_pdf_metadata(file_key)
        print ("Metadata >>", metadata)
        
        # Si el evento no traía URL, pero la encontramos en la metadata, la reasignamos
        if metadata and file_url == DEFAULT_URL_NOT_FOUND:
            file_url = metadata.get('url', DEFAULT_URL_NOT_FOUND)
